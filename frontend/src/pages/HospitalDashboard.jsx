    ...
    <div style={styles.wrapper}>
      <div style={styles.sidebar} className="glass-card">
        <div style={styles.sidebarTop}>
          ...
          <h2 style={styles.sidebarTitle}>Portal</h2>
          <p style={styles.welcome}>Logged in as<br /><strong>{user?.name}</strong></p>
        </div>
        <nav style={styles.nav}>
          ...
          <button onClick={() => setActiveTab('requests')} style={{...styles.navItem, ...(activeTab === 'requests' && styles.navItemActive)}}>
            <span>📩</span> Patient Requests {requests.length > 0 && <span style={styles.badge}>{requests.length}</span>}
          </button>
          <button onClick={() => setActiveTab('profile')} style={{...styles.navItem, ...(activeTab === 'profile' && styles.navItemActive)}}>
...
const styles = {
  wrapper: { minHeight: '100vh', display: 'flex', alignItems: 'stretch' },
  sidebar: { width: '260px', minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: '30px 20px', position: 'sticky', top: 0 },
  sidebarTop: { marginBottom: '40px' },
  sidebarTitle: { fontSize: '1.6rem', fontWeight: '800', margin: '10px 0 6px', color: '#fff' },
  nav: { display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 },
  navItem: { display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', borderRadius: '12px', border: 'none', background: 'transparent', color: '#94a3b8', fontWeight: '600', fontSize: '0.95rem', cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s' },
  navItemActive: { background: '#fff', color: '#0f172a', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' },
  badge: { marginLeft: 'auto', background: '#dc2626', color: '#fff', borderRadius: '100px', padding: '2px 8px', fontSize: '0.75rem' },
  mainContent: { flex: 1, padding: '40px 30px', overflowY: 'auto' },
  ...
  welcome: { color: '#94a3b8', fontSize: '0.95rem', fontWeight: '300', marginTop: '4px' },
